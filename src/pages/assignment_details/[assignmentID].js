                            ...
                            axios(config)
                              .then(async function (response) {
                                fileUrl =
                                  "https://assignmentsanta.blob.core.windows.net/assignment-dscp/" +
                                  encodeURIComponent(
                                    inputFileOperatorExpert.current.files[0]
                                      .name
                                  );
                              ...
                              axios(config)
                                .then(async function (response) {
                                  ...
                                  if (fileUrl === "") {
                                    ...
                                    const response = await axios.post(
                                      apiUrl +
                                        "/assignment/comments/QCToExpert",
                                      {
                                        assignmentId:
                                          navigate.query.assignmentID,
                                        expertId: assignment.assignedExpert,
                        ...
                        onClick={async () => {
                          if (assignment.assignedSales !== id) {
                            ...
                            let Regex =
                              /\b[\+]?[(]?[0-9]{2,6}[)]?[-\s\.]?[-\s\/\.0-9]{3,15}\b/m;
                            let textInput =
                              document.getElementById("addChatSales");
                            if (
                              textInput.value !== "" &&
                              textInput.value !== undefined
                            ) {
...
export async function getStaticPaths() {
  let config = {
    headers: {
      Authorization: `Bearer eyJhbGciOiJIUzI1NiJ9.YXNzaWdubWVudHNhbnRhQGdtYWlsLmNvbQ.BBLUnI7QRpKeWKshUDdNK9wUz6dYo4TrxmvCWAI_EjA`,
    },
  };
