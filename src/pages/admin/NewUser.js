...
export const NewUser = () => {
    ...
    const [SendSubjectData, setSendSubjectData] = useState([])
        ...
        try {
            ...
            if (data.length !== 0) {
                for (let index = 0; index < data.length; index++) {
                    ...
                    // console.log(tempList, 'subject user')
    ...
    const getsubjects = (data) => {
        let val = document.getElementsByClassName('sub');
        let tempUserData = [];
        console.log(val)
        console.log(val[0].value)
        for (let i = 0; i < val.length; i++) {
            if (val[i].checked) {
                tempUserData.push(val[i].value)
                // tempUserData = [val[i].value];
                console.log(tempUserData)
            }
            setSendSubjectData(tempUserData)
        }
    }
        useEffect(
            () => {
                                                ...
                                                <Stack spacing={4} direction='column'>
                                                    {
                                                        subjectcheck.map((res) => {
                                                            return (<>
                                                                <FlexDiv>
                                                                    <input type='checkbox' value={res} onChange={() => getsubjects(res)} mr={2} className='sub' />
                                                                    <div ml='4'>{res}</div >
                                                                </FlexDiv>
                                                            </>
                                                            )
                                            ...
                                            if (
                                                ...
                                                try {
                                                    ...
                                                    if (responseExpert.data.success) {
                                                        console.log(responseExpert, 'success expert add')
