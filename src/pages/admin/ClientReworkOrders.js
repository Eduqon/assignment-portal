...
function ClientReworkOrders() {
    ...
    async function removeExpert(index) {
        try {
            ...
            const response = await axios.post(apiUrl + '/expert/remove/ask',
                {
        ...
        if (qcComments.value === '') {
            ...
            if (fileUrl === '') {
                try {
                    ...
                    const responseQcNote = await axios.post(apiUrl + '/assignment/comments/QCToExpert',
                        {
                            "assignmentId": assignments[index].id,
                ...
                try {
                    ...
                    const responseQcNoteFile = await axios.post(apiUrl + '/assignment/comments/QCToExpert',
                        {
                            "assignmentId": assignments[index].id,
                            "expertId": assignments[index].assignedExpert,
                            "commentsFromQC": {
    ...
    async function uploadFile(blobName, filePath) {
        ...
        axios(config)
            .then(function (response) {
                ...
                setFileUrl('https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(blobName));
    ...
    async function openSubmissionsModal(index) {
        ...
        try {
            ...
            const response = await axios.get(apiUrl + '/assignment/submissions/fetch?assignment_id=' + assignments[index].id, config
            );
                        ...
                        <Table variant='simple' size="sm">
                            ...
                            <Tbody>
                                {(submissions.length === 0) ? <></> : submissions.map(
                                    (submission, index) => <Tr key={submission._id}>
                                        <Td maxW={'100px'}><Link isExternal={true} href={submission.url}>{submission.name}</Link></Td>
                                        <Td>{submission.category}</Td>
                                        <Td>{submission.createdAt}</Td>
                                        <Td color={((new Date(submission.createdAtNF) - new Date(assignments[selectedIndex].expertDeadlineNF) <= 0)) ? 'green' : 'red'}>{
                                            (
                                                (new Date(submission.createdAtNF) - new Date(assignments[selectedIndex].expertDeadlineNF) <= 0)
                                                    ? "On Time" :
                                                    "Late"
                                            )
                                        }</Td>
    ...
    async function _fetchAssignments() {
        try {
            ...
            const response = await axios.get(apiUrl + '/assignment/fetch?status=Client%20Rework', config);
                    ...
                    <Tbody>
                        {assignments.map(
                            (assignment, index) => <Tr key={assignment.id}>
                                ...
                                <Td>
                                    <HStack>
                                        <Button onClick={async () => openSubmissionsModal(index)}>Submissions from Experts</Button>
                                        <Button onClick={async () => markAsProofRead(index)}>Mark as Proof Read</Button>
                                        <Button color={'red'} onClick={async () => openReworkModal(index)}>Send for Rework</Button>
...
export default ClientReworkOrders;