...
import { RepeatIcon, ArrowForwardIcon, AttachmentIcon, AddIcon } from "@chakra-ui/icons";
...
function AssignmentDetails() {
    ...
    async function _fetchQuotations() {
        try {
            ...
            const response = await axios.get(apiUrl + '/assignment/quotes/fetch?assignment_id=' + params.assignmentID, config
            );
    ...
    async function _fetchAssignmentFiles() {
        ...
        const response = await axios.get(apiUrl + '/assignment/assignmentFiles/fetch?assignment_id=' + params.assignmentID, config);
        ...
        if (data !== null) {
            for (let index = 0; index < data.length; index++) {
                assignmentFilesList.push({
                    ...
                    createdAt: new Date(data[index].createdAt).toLocaleTimeString() + ", " + new Date(data[index].createdAt).toDateString(),
                });
            }
        }
        else {
    ...
    async function _fetchSubmissions() {
        ...
        const response = await axios.get(apiUrl + '/assignment/submissions/fetch?assignment_id=' + params.assignmentID, config);
    ...
    async function _fetchActions() {
        ...
        const response = await axios.get(apiUrl + '/assignment/activity/fetch?assignment_id=' + params.assignmentID, config);
        ...
        if (data !== null) {
            for (let index = 0; index < data.activity.length; index++) {
                actionsList.push({
                    ...
                    at: new Date(data.activity[index].createdAt).toLocaleTimeString() + ", " + new Date(data.activity[index].createdAt).toDateString(),
                });
            }
        }
        else {
    ...
    async function _fetchAssignmentQcNotes() {
        ...
        const response = await axios.get(apiUrl + '/assignment/qc-comments/fetch?assignment_id=' + params.assignmentID, config);
        let data = await response.data.result;
        qcNotesList = [];
        if (data !== null) {
    ...
    async function _addAssignmentQcNote() {
        ...
        if (qcBox.value === "") {
            ...
            const response = await axios.post(apiUrl + '/assignment/comments/QCToExpert',
                {
    ...
    async function _fetchAssignmentStickyNotes() {
        ...
        const response = await axios.get(apiUrl + '/assignment/comments/fetch?assignment_id=' + params.assignmentID, config);
        let data = await response.data.result;
        stickyNotesList = [];
        if (data !== null) {
    ...
    async function _addAssignmentStickyNote() {
        ...
        if (stickyBox.value === "") {
            ...
            const response = await axios.post(apiUrl + '/assignment/comments/operatorToExpert',
                {
                    "assignmentId": params.assignmentID,
                    "expertId": assignment.assignedExpert,
                    "notesFromOperator": {
    ...
    async function _fetchAssignmentDetails() {
        try {
            ...
            if (data.length !== 0) {
                ...
                await _fetchSalesChatView(data[0].client_id, data[0].assignedSales, data[0]._id);
                await _fetchSalesChat(data[0].client_id, data[0]._id);
                await _fetchOperatorExpertChatView(data[0].assignedExpert, data[0].assignedOperator, data[0]._id);
                await _fetchOperatorExpertChat(data[0].assignedExpert, data[0]._id);
                await _fetchQcExpertChatView(data[0].assignedExpert, data[0].assignedQC, data[0]._id);
    ...
    async function _fetchOperatorExpertChatView(expertEmail, operatorEmail, assignment_id) {
        if (localStorage.getItem('userRole') === 'Admin' || localStorage.getItem('userRole') === 'Super Admin') {
            try {
    ...
    async function uploadFile(blobName, filePath) {
        ...
        axios(config)
            .then(function (response) {
                console.log(response);
                setAssignmentFileUrl('https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(blobName));
                ...
                <ModalContent>
                    ...
                    <ModalBody>
                        <Table variant='simple' size="sm">
                            ...
                            <Tbody>
                                {(assignmentFiles.length === 0) ? <></> : assignmentFiles.map(
                                    (assignmentFile, index) => <Tr key={index}>
                                        <Td maxW={'100px'}><Link isExternal={true} href={assignmentFile.url}>{assignmentFile.name}</Link></Td>
                                                ...
                                                onChange={
                                                    (e) => {
                                                        (e.target.checked) ?
                                                            ...
                                                            checkedListTemp.splice(checkedListTemp.indexOf(e.target.value))
                                                    }
                                                }
                                            />
                                        </Td>
                    ...
                    <ModalFooter>
                        <HStack>
                            <Checkbox value={isCustomMail} onChange={
                                (e) => {
                                    (e.target.checked) ?
                            ...
                            <Input display={(isCustomMail) ? 'block' : 'none'} type={'text'} id={'customMail'} />
                            <Button
                                onClick={async () => {
                                    if (checkedListTemp.length === 0) {
                                        ...
                                        if (isCustomMail) {
                                            ...
                                            if (customMail.value === "") {
                                                ...
                                                try {
                                                    let response = await axios.post(apiUrl + "/assignment/final-delivery",
                                                        {
                ...
                <ModalContent>
                                ...
                                <InputRightAddon>
                                    ...
                                    <input
                                        ...
                                        onChange={async () => {
                                            ...
                                            await uploadFile(inputRef.current.files[0].name, inputRef.current.files[0]);
                                        }}
                                        ref={inputRef}
                                        style={{ display: "none" }}
                                    />
                    ...
                    <ModalFooter>
                        <Button
                            onClick={async () => {
                                if (isUploading) {
                                    window.alert("File is still uploading... Please wait and try again");
                                } else {
                                    ...
                                    if (fileCategory.value === '') {
                                        ...
                                        try {
                                            let response = await axios.post(apiUrl + "/assignment/assignmentFiles",
                                                {
                                                    "_id": params.assignmentID,
                                                    "files": [
                                                        {
        ...
        if (assignment == undefined) {
            ...
            return (
                <VStack alignItems={'start'} margin={5}>
                    ...
                    <SendToClientModal />
                    <HStack>
                        <Button onClick={async () => { navigate("/admin/portal"); }}>Back to Orders</Button>
                        <Button leftIcon={<RepeatIcon />} onClick={async () => { _fetchAssignmentDetails(); }}>Refresh</Button>
                    </HStack>
                    <Wrap  width="100%">
                            ...
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                    {(actions.length === 0) ? <></> : actions.map(
                                        (action, index) => <Box key={action._id}>
                                            <VStack alignItems={'start'}>
                                                <HStack>
                                                    {(index === 0) ? <motion.div animate={{ scale: 0.5 }} transition={{
                                                        repeat: Infinity,
                                                        repeatType: 'reverse',
                                                        duration: 1
                                                    }}>
                                                        ...
                                                        <Circle
                                                            size={'10px'}
                                                            bgColor={'blue.600'}></Circle>}
                                                    <Text>{action.at + ": "}</Text>
                                                    <Text fontWeight={'bold'}>{(action.role === 'Client') ? "Client" : action.uid}</Text>
                                                </HStack>
                        ...
                        <Box display={(userRole === "Admin" || userRole === 'Super Admin') ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'} >
                            <Box bgColor="gray.200" p={4}>
                                <Heading fontSize={'xl'}>Expert Quotations</Heading>
                            </Box>
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                    {(quotations.length === 0) ? <></> : quotations.map(
                                        (quotation, index) => <Box key={index}>
                                            <VStack alignItems={'start'} marginBottom={'20px'}>
                                                <VStack alignItems={'flex-start'}>
                                                    <Text color={(quotation._id === assignment.assignedExpert) ? 'red' : ''} fontWeight={'bold'}>Expert: {quotation._id}</Text>
                        ...
                        <Box borderWidth='1px' borderRadius='md' width={'xl'} >
                            <Box display={'flex'} alignItems={'center'} bgColor="gray.200" p={2}>
                                ...
                                <Spacer />
                                <Button marginRight={'10px'} onClick={() => { openSendToClientModal() }}>
                                    Send to Client
                                </Button>
                                <Button onClick={() => { openAssignmentFileUploadModal() }}>
                                    Upload
                                </Button>
                            </Box>
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                    {(assignmentFiles.length === 0) ? <></> : assignmentFiles.map(
                                        (assignmentFile, index) => <Box key={index}>
                                            <VStack alignItems={'start'} marginBottom={'20px'}>
                                                <HStack>
                                                ...
                                                <HStack>
                                                    ...
                                                    <Link isExternal={true} href={assignmentFile.url}>{assignmentFile.name}</Link>
                                                </HStack>
                                                <HStack>
                                                    <Text fontWeight={'bold'}>User: </Text>
                                                    <Text fontWeight={'bold'}>{assignmentFile.userId}</Text>
                                                </HStack>
                                            </VStack>
                                        </Box>
                                    )}
                        ...
                        <Box display={(userRole === "QC" || userRole === "Admin" || userRole === "Super Admin" || userRole === "Operator" || userRole === "Sales") ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'} >
                            <Box bgColor="gray.200" p={4}>
                                <Heading fontSize={'xl'}>Expert Submissions</Heading>
                            </Box>
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                    {(submissions.length === 0) ? <></> : submissions.map(
                                        (submission, index) => <Box key={submission._id}>
                                            <VStack alignItems={'start'} marginBottom={'20px'}>
                                                <HStack>
                                                    <Text fontWeight={'bold'}>• Category: </Text>
                                                ...
                                                <HStack>
                                                    <Text fontWeight={'bold'}>File Name: </Text>
                                                    <Link isExternal={true} href={submission.url}>{submission.name}</Link>
                                                </HStack>
                                                <HStack>
                        ...
                        <Box display={(userRole === "Operator" && assignment.assignedExpert !== undefined) ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'}>
                            ...
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                ...
                                <InputGroup>
                                    ...
                                    />
                                    <Input type='file' id='addFileOperatorExpert' onChange={async () => {
                                        let fileUrl = '';
                                        if (inputFileOperatorExpert) {
                                            ...
                                            try {
                                                var config = {
                                                    method: 'put',
                                                    url: 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileOperatorExpert.current.files[0].name) + "?" + token,
                                                    headers: {
                                                        'x-ms-blob-type': 'BlockBlob'
                                                    },
                                                ...
                                                axios(config)
                                                    .then(async function (response) {
                                                        fileUrl = 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileOperatorExpert.current.files[0].name);
                                                        const message = await updateDoc(doc(db, "chat", assignment.assignedExpert + '_' + id + "_" + assignment.id), {
                                                            conversation:
                                                                arrayUnion({
                                                                    msg: fileUrl,
                                                                    time: Date.now(),
                                    ...
                                    }} ref={inputFileOperatorExpert} style={{ display: 'none' }} />
                                    ...
                                    <InputRightElement h={'full'}>
                                        <Button id='sendButton' onClick={async () => {
                                            ...
                                            let textInput = document.getElementById('addChatOperatorExpert');
                                            if (textInput.value !== '' && textInput.value !== undefined) {
                                                if (Regex.test(textInput.value)) {
                                                    window.alert("Sharing Phone Numbers through Chat is not allowed");
                                                } else {
                                                    const message = await updateDoc(doc(db, "chat", assignment.assignedExpert + '_' + id + "_" + assignment.id), {
                                                        conversation:
                                                            arrayUnion({
                        ...
                        <Box borderWidth='1px' borderRadius='md' width={'xl'} >
                            ...
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                    {(stickyNotes.length === 0) ? <></> : stickyNotes.map(
                                        (stickyNote, index) => <Box key={stickyNote.id + stickyNote.comment}>
                                            <Text fontWeight={'bold'}>{stickyNote.name + ":"}</Text>
                                            <Text display={(stickyNote.comment.substring(0, 62) == "https://assignmentsanta.blob.core.windows.net/assignment-dscp/") ? 'none' : 'flex'}>{stickyNote.comment}</Text>
                                            <Link color={'blue'} fontWeight={'bold'} display={(stickyNote.comment.substring(0, 62) == "https://assignmentsanta.blob.core.windows.net/assignment-dscp/") ? 'flex' : 'none'} href={stickyNote.comment}>{stickyNote.comment.substring(62)}</Link>
                                            {/* <Divider /> */}
                                        </Box>
                                    )}
                                </VStack>
                                <InputGroup visibility={(userRole === "Operator" || userRole === "Admin" || userRole === "Super Admin") ? "visible" : "hidden"}>
                                    <Input
                                        type="text"
                                        id="addSticky"
                                    />
                                    <Input type='file' id='addFileStickyNotes' onChange={async () => {
                                        let fileUrl = '';
                                        if (inputFileStickyNotes) {
                                            try {
                                                var config = {
                                                    method: 'put',
                                                    url: 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileStickyNotes.current.files[0].name) + "?" + token,
                                                    headers: {
                                                        'x-ms-blob-type': 'BlockBlob'
                                                    },
                                                ...
                                                axios(config)
                                                    .then(async function (response) {
                                                        fileUrl = 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileStickyNotes.current.files[0].name);
                                                        let userToken = localStorage.getItem("userToken");
                                                        let userEmail = localStorage.getItem("userEmail");
                                                        let userName = localStorage.getItem("userName");
                                    ...
                                    }} ref={inputFileStickyNotes} style={{ display: 'none' }} />
                                    <InputLeftElement h={'full'}>
                                        <Button id='attachButton' onClick={async () => {
                        ...
                        <HStack display={(userRole === "QC") ? "block" : "none"}>
                            <Box borderWidth='1px' borderRadius='md' width={'xl'} >
                                ...
                                <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                    <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                        {(qcNotes.length === 0) ? <></> : qcNotes.map(
                                            (qcNote, index) => <Box key={qcNote.id + qcNote.comment}>
                                                <Text fontWeight={'bold'}>{qcNote.name + ":"}</Text>
                                                <Text display={(qcNote.comment.substring(0, 62) == "https://assignmentsanta.blob.core.windows.net/assignment-dscp/") ? 'none' : 'flex'}>{qcNote.comment}</Text>
                                                <Link color={'blue'} fontWeight={'bold'} display={(qcNote.comment.substring(0, 62) == "https://assignmentsanta.blob.core.windows.net/assignment-dscp/") ? 'flex' : 'none'} href={qcNote.comment}>{qcNote.comment.substring(62)}</Link>
                                                {/* <Divider /> */}
                                            </Box>
                                        )}
                                    </VStack>
                                    <InputGroup visibility={(userRole === "QC") ? "visible" : "hidden"}>
                                        <Input
                                            type="text"
                                            id="addQc"
                                        />
                                        <Input type='file' id='addFileQCNotes' onChange={async () => {
                                            let fileUrl = '';
                                            if (inputFileQCNotes) {
                                                try {
                                                    var config = {
                                                        method: 'put',
                                                        url: 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileQCNotes.current.files[0].name) + "?" + token,
                                                        headers: {
                                                            'x-ms-blob-type': 'BlockBlob'
                                                        },
                                                    ...
                                                    axios(config)
                                                        .then(async function (response) {
                                                            fileUrl = 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileQCNotes.current.files[0].name);
                                                            let userToken = localStorage.getItem("userToken");
                                                            let userEmail = localStorage.getItem("userEmail");
                                                            let userName = localStorage.getItem("userName");
                                        ...
                                        }} ref={inputFileQCNotes} style={{ display: 'none' }} />
                                        <InputLeftElement h={'full'}>
                                            <Button id='attachButton' onClick={async () => {
                        ...
                        <Box display={(userRole === "QC" && id === assignment.assignedQC && assignment.assignedExpert !== undefined) ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'}>
                            <Box p={4} bgColor="gray.200">
                                <HStack>
                            ...
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                ...
                                <InputGroup>
                                    <Input
                                        ...
                                        id="addChatQCExpert"
                                    />
                                    <Input type='file' id='addFileQCExpert' onChange={async () => {
                                        let fileUrl = '';
                                        if (inputFileQCExpert) {
                                            ...
                                            try {
                                                var config = {
                                                    method: 'put',
                                                    url: 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileQCExpert.current.files[0].name) + "?" + token,
                                                    headers: {
                                                        'x-ms-blob-type': 'BlockBlob'
                                                    },
                                                ...
                                                axios(config)
                                                    .then(async function (response) {
                                                        fileUrl = 'https://assignmentsanta.blob.core.windows.net/assignment-dscp/' + encodeURIComponent(inputFileQCExpert.current.files[0].name);
                                                        const message = await updateDoc(doc(db, "chat", assignment.assignedExpert + '_' + id + "_" + assignment.id), {
                                                            conversation:
                                                                arrayUnion({
                                                                    msg: fileUrl,
                                    ...
                                    }} ref={inputFileQCExpert} style={{ display: 'none' }} />
                                    <InputLeftElement h={'full'}>
                                        <Button id='attachButton' onClick={async () => {
                        ...
                        <Box display={(userRole === "Sales") ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'}>
                            <Box p={4} bgColor="gray.200">
                                <HStack>
                                    <Heading fontSize={'xl'}>Sales Chat with Client</Heading>
                                    <Spacer />
                                    <Text
                                        visibility={(userRole === "Sales" && assignment.assignedSales !== id) ? 'visible' : 'hidden'}
                                        color={'red'}
                                        onClick={
                                            async () => {
                                                let userToken = localStorage.getItem('userToken');
                                                ...
                                                window.alert("Override Complete. You are now Assigned to this Assignment")
                                            }}>
                                        Override
                                    </Text>
                                </HStack>
                                            ...
                                            try {
                                                ...
                                                axios(config)
                                                    .then(async function (response) {
                                                        ...
                                                        const message = await updateDoc(doc(db, "chat", clientId + '_' + id + "_" + assignment.id), {
                                                            conversation:
                                                                arrayUnion({
                                                                    msg: fileUrl,
                                                                    time: Date.now(),
                                    ...
                                    <InputRightElement h={'full'}>
                                        <Button id='sendButton' onClick={async () => {
                                            if (assignment.assignedSales !== id) {
                                                window.alert("You are not assigned to this Assignment!!! You will have to override first to continue");
                                            }
                                            else {
                                                let Regex = /\b[\+]?[(]?[0-9]{2,6}[)]?[-\s\.]?[-\s\/\.0-9]{3,15}\b/m;
                        ...
                        <Box display={(userRole === "Admin" || userRole === "Super Admin") ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'}>
                            <Box p={4} bgColor="gray.200">
                                <HStack>
                                    <Heading fontSize={'xl'}>Sales Chat with Client (Admin View)</Heading>
                                    <Spacer />
                                </HStack>
                            </Box>
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                                <VStack overflowY={'scroll'} alignItems={'start'} width={'100%'}>
                                    {
                                        salesViewChat.map(
                                            (messageItem, index) =>
                                                <>
                                                    <Text fontWeight={'bold'} maxWidth={'100%'}>{messageItem.user}:</Text>
                                                    <Box
                                                        display={(messageItem.type === 'TEXT') ? "flex" : (messageItem.type === 'MEDIA') ? "flex" : "none"}
                                                        flexWrap={true}
                                                        padding={2}
                        ...
                        <Box display={(userRole === "Admin" || userRole === "Super Admin") ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'}>
                            <Box p={4} bgColor="gray.200">
                                <HStack>
                                    <Heading fontSize={'xl'}>Operator Chat with Expert (Admin View)</Heading>
                                    <Spacer />
                                </HStack>
                            </Box>
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
                        ...
                        <Box display={(userRole === "Admin" || userRole === "Super Admin") ? "block" : "none"} borderWidth='1px' borderRadius='md' width={'xl'}>
                            <Box p={4} bgColor="gray.200">
                                <HStack>
                                    <Heading fontSize={'xl'}>QC Chat with Expert (Admin View)</Heading>
                                    <Spacer />
                                </HStack>
                            </Box>
                            <VStack alignItems={'start'} justifyContent={'space-between'} margin={3} minH={'sm'} maxH={'sm'}>
